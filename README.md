...
菜单栏图标太多放不下、被系统裁掉、找不到也点不到？`⌥⌘B` 唤起一个网格面板：

```
┌──────────────────────────────────────────────┐
│ 菜单栏图标 12 个 · 3 个屏幕上放不下             │
│                                              │
│  ▣🟠    ▣      ▣      ▣                       │
│ ToDesk  飞书   Kaka   微信                     │
│ 已连接  未读3                                  │
├──────────────────────────────────────────────┤
│   12%      46%     ↓4KB/s    ↓18KB/s         │
│ CPU·负载3.4 内存·58/128GB 磁盘·可用1.4TB 网络·en0│
└──────────────────────────────────────────────┘
```

- **🟠 橙点** = 被系统裁掉、屏幕上根本看不到的那些，排在最前面
- **点磁贴** = 替你按下真实的菜单栏项，弹出的是那个 App **自己原汁原味的菜单**
- 底部是 CPU / 内存 / 磁盘 / 网络实时状态（**零权限**）
- 还能把选定的一组图标折叠起来（⌘ 拖动边界标记决定分组）

**默认关闭**，从菜单栏 LidAwake →「菜单栏折叠」启用。列出和点击图标需要**辅助功能**权限
（就是 Raycast / Rectangle 要的那个），折叠和系统状态**不需要任何权限**。

技术细节、权限边界、以及"为什么拿不到别人的图标图像"见 [docs/MENUBAR.md](docs/MENUBAR.md)。
